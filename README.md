...
```bash
python -m pip install -r "c:\repos\ephem\requirements.txt"
```
...
```bash
python "c:\repos\ephem\oem_to_radec.py" ^
  --oem "c:\repos\ephem\Artemis_II_OEM_2026_04_02_to_EI_v3.asc" ^
...
Use `--output-to-csv <output_dir>` (also accepts the alias `--ouput-to-csv`).
...
python "c:\repos\ephem\oem_to_radec.py" ^
  ...
  --lat 53.8616 --lon -6.5378 --height-m 30 ^
  --interval 10 --positions 20 ^
  --output-to-csv "c:\repos\ephem\csv_out"
```

### Run tests

```bash
python -m pip install -r "c:\repos\ephem\requirements-dev.txt"
python -m pytest -q "c:\repos\ephem\tests"
```
